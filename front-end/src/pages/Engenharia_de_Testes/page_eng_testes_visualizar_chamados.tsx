...
export function VisualizarChamados() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await fetch('http://10.161.100.176:5000/api/chamados', {
                    method: 'GET',
                    headers: {
                        'Content-Type': 'application/json'
                    }
                });
                const responseAtendidos = await fetch(`http://10.161.100.176:5000/api/chamadosatendidos?dataInicial=${encodeURIComponent(dataInicialString)}&dataFinal=${encodeURIComponent(dataFinalString)}`, {
                    method: 'GET',
                    headers: {
                        'Content-Type': 'application/json'
                    },
