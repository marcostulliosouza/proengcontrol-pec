...
const Header: React.FC<HeaderProps> = ({ }) => {
    ...
    // Função para extrair as iniciais do usuário
    ...
    return (
        <header className="bg-pec text-white p-4 flex items-center justify-between relative pl-16">
            <button className="flex items-center gap-2" onClick={() => navigate('/home')}>
                <img src={logo} alt="PEC" className="w-6 h-6" />
                <h1 className="text-xl font-semibold">PEC</h1>
            </button>
            <nav className="flex items-center gap-4">
                <button onClick={() => navigate('/about')} className="text-white hover:text-gray-200">
                    Sobre
                </button>
                <button onClick={() => navigate('/contact')} className="text-white hover:text-gray-200">
                    Links
                </button>
